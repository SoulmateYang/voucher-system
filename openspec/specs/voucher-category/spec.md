...
系统 SHALL 提供分类 CRUD 接口，管理员可创建、查看、编辑、删除自定义卡券分类。

#### Scenario: Create a new category
- **WHEN** 管理员调用 `POST /api/v1/categories` 并传入 `{ "name": "餐饮类" }`
- **THEN** 系统创建分类并返回包含 `id`、`name`、`sortOrder` 的分类对象

#### Scenario: List all categories
- **WHEN** 管理员调用 `GET /api/v1/categories`
- **THEN** 系统返回所有分类列表，按 `sortOrder` 升序排列

#### Scenario: Update category name
- **WHEN** 管理员调用 `PUT /api/v1/categories/{id}` 并传入 `{ "name": "美食餐饮" }`
- **THEN** 系统更新分类名称并返回更新后的对象
...
系统 SHALL 提供接口将单张券归入指定分类或从分类中移除。
...
系统 SHALL 提供批量接口，允许管理员将多张券一次性归入同一分类。
...
- **WHEN** 管理员在券码列表选择"餐饮类"分类筛选
- **THEN** 列表仅展示 `category_id` 匹配该分类的券

#### Scenario: Show category name in voucher list row
- **WHEN** 管理员查看券码列表
- **THEN** 每行显示该券所属分类名称，未归类的券显示 "—"

#### Scenario: Admin UI category management page
- **WHEN** 管理员点击侧边栏"分类管理"
- **THEN** 进入分类管理页面，展示分类列表，每行有编辑和删除按钮
- **THEN** 顶部有"新建分类"按钮，弹出输入框
- **THEN** 列表支持拖拽排序（sortOrder）

#### Scenario: Admin UI batch move
- **WHEN** 管理员在券码列表勾选多条券码，点击"批量移动"
- **THEN** 弹出分类选择器，选择目标分类后确认，所有选中券归入该分类
